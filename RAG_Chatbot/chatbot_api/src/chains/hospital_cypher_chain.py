...
# Set up connection to our AuraDB.
graph = Neo4jGraph(
...
# Sync any recent changes to our instance.
graph.refresh_schema()

cypher_prompt_template_text = """
Task:
...
cypher_prompt_template = PromptTemplate(
    input_variables=["schema", "question"], template=cypher_prompt_template_text
)

qa_prompt_template_text = """You are an assistant that takes the results
...
qa_prompt_template = PromptTemplate(
    input_variables=["context", "question"], template=qa_prompt_template_text
)
...
hospital_cypher_chain = GraphCypherQAChain.from_llm(
    ...
    graph=graph,
    # Set to True to see generated Cypher.
    verbose=False,
    qa_prompt=qa_prompt_template,
    cypher_prompt=cypher_prompt_template,
    # Only corrects simple syntax errors that are easily detectable using regular expressions.
    validate_cypher=True,
    # The number of query results to include in qa_prompt.
    top_k=100,
)
